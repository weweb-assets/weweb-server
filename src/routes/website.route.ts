...
export default (app: Application) => {
    app.route(`${process.env.SERVER_PATH}/:lang(${LANGS})?/:path(*)?/data.json`).get(
        websiteMdwl.ensureWebsite,
        websiteMdwl.ensurePage,
        websiteMdwl.ensureAuth,
        websiteCtrl.getDataFileV1
    )
    app.route(`${process.env.SERVER_PATH}/public/data/:pageId(*).json`).get(
        websiteMdwl.ensureWebsite,
        websiteMdwl.ensurePageFromId,
        websiteMdwl.ensureAuth,
        websiteCtrl.getDataFile
    )
    app.route(`${process.env.SERVER_PATH}/:path(*.*)`).get(
        websiteMdwl.ensureWebsite,
        websiteCtrl.getFile,
    )
    app.route(`${process.env.SERVER_PATH}/:lang(${LANGS})?/:path(*)`).get(
        websiteMdwl.ensureWebsite,
